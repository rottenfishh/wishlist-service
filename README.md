...
Все параметры сервиса задаются через переменные окружения.

Их список указан в [.env.example](./.env.example) Для локального переопределения значений можно создать `.env` на его основе.
## Github actions
...
```
Полная интерактивная документация доступна в Swagger UI после запуска сервиса.
